...
export class OffChainParticipant {
  ...
  validate(compatible = false) {
    ...
    new SEC1EncodedPubKey(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
  ...
  getPubKey() {
    ...
    return new SEC1EncodedPubKey(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
...
export function SerializeOffChainParticipant(value) {
  const buffers = [];
  buffers.push(SerializeSEC1EncodedPubKey(value.pub_key));
  buffers.push(SerializeScript(value.payment_script));
  buffers.push(SerializeScript(value.unlock_script));
...
export class SEC1EncodedPubKey {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
  ...
  validate(compatible = false) {
    assertDataLength(this.view.byteLength, 33);
  }
...
export function SerializeSEC1EncodedPubKey(value) {
  const buffer = assertArrayBuffer(value);
  assertDataLength(buffer.byteLength, 33);
  return buffer;
}

export class CKByteDistribution {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
  ...
  validate(compatible = false) {
    assertDataLength(this.view.byteLength, Uint64.size() * 2);
    for (let i = 0; i < 2; i++) {
      const item = this.indexAt(i);
      item.validate(compatible);
    }
  ...
  indexAt(i) {
    return new Uint64(this.view.buffer.slice(i * Uint64.size(), (i + 1) * Uint64.size(), { validate: false }));
  }
...
export function SerializeCKByteDistribution(value) {
  const array = new Uint8Array(Uint64.size() * value.length);
  for (let i = 0; i < value.length; i++) {
    const itemBuffer = SerializeUint64(value[i]);
    array.set(new Uint8Array(itemBuffer), i * Uint64.size());
  }
  return array.buffer;
}

export class SUDTDistribution {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
  ...
  validate(compatible = false) {
    assertDataLength(this.view.byteLength, Uint128.size() * 2);
    for (let i = 0; i < 2; i++) {
      const item = this.indexAt(i);
      item.validate(compatible);
    }
  ...
  indexAt(i) {
    return new Uint128(this.view.buffer.slice(i * Uint128.size(), (i + 1) * Uint128.size(), { validate: false }));
  }

  static size() {
    return Uint128.size() * 2;
  }
}

export function SerializeSUDTDistribution(value) {
  const array = new Uint8Array(Uint128.size() * value.length);
  for (let i = 0; i < value.length; i++) {
    const itemBuffer = SerializeUint128(value[i]);
    array.set(new Uint8Array(itemBuffer), i * Uint128.size());
  }
  return array.buffer;
}

export class SUDTAllocation {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
  ...
  validate(compatible = false) {
    const offsets = verifyAndExtractOffsets(this.view, 0, true);
    for (let i = 0; i < offsets.length - 1; i++) {
      new SUDTBalances(this.view.buffer.slice(offsets[i], offsets[i + 1]), { validate: false }).validate();
    }
  }
  ...
  indexAt(i) {
    ...
    return new SUDTBalances(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
}

export function SerializeSUDTAllocation(value) {
  return serializeTable(value.map(item => SerializeSUDTBalances(item)));
}

export class SUDTAsset {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
...
export function SerializeSUDTAsset(value) {
  const buffers = [];
  buffers.push(SerializeScript(value.type_script));
  buffers.push(SerializeUint64(value.max_capacity));
  return serializeTable(buffers);
}

export class SUDTBalances {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
  ...
  validate(compatible = false) {
    const offsets = verifyAndExtractOffsets(this.view, 0, true);
    new SUDTAsset(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
    new SUDTDistribution(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
  }
  ...
  getAsset() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.getUint32(start + 4, true);
    return new SUDTAsset(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
  ...
  getDistribution() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.byteLength;
    return new SUDTDistribution(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
}

export function SerializeSUDTBalances(value) {
  const buffers = [];
  buffers.push(SerializeSUDTAsset(value.asset));
  buffers.push(SerializeSUDTDistribution(value.distribution));
  return serializeTable(buffers);
}
...
export class Balances {
  ...
  validate(compatible = false) {
    const offsets = verifyAndExtractOffsets(this.view, 0, true);
    new CKByteDistribution(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
    new SUDTAllocation(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
  }
  ...
  getCkbytes() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.getUint32(start + 4, true);
    return new CKByteDistribution(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
  ...
  getSudts() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.byteLength;
    return new SUDTAllocation(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
}

export function SerializeBalances(value) {
  const buffers = [];
  buffers.push(SerializeCKByteDistribution(value.ckbytes));
  buffers.push(SerializeSUDTAllocation(value.sudts));
  return serializeTable(buffers);
}
...
export function SerializeTrue(value) {
  const buffer = assertArrayBuffer(value);
...
export function SerializeFalse(value) {
  const buffer = assertArrayBuffer(value);
  assertDataLength(buffer.byteLength, 1);
  return buffer;
}
...
export class Bool {
  ...
  validate(compatible = false) {
    ...
    switch (t) {
    case 0:
      new True(this.view.buffer.slice(4), { validate: false }).validate();
      break;
    case 1:
      new False(this.view.buffer.slice(4), { validate: false }).validate();
      break;
    default:
  ...
  value() {
    ...
    switch (t) {
    case 0:
      return new True(this.view.buffer.slice(4), { validate: false });
    case 1:
      return new False(this.view.buffer.slice(4), { validate: false });
    default:
      throw new Error(`Invalid type: ${t}`);
    }
  }
...
export function SerializeBool(value) {
  ...
  case "True":
    {
      const itemBuffer = SerializeTrue(value.value);
  ...
  case "False":
    {
      const itemBuffer = SerializeFalse(value.value);
      const array = new Uint8Array(4 + itemBuffer.byteLength);
      const view = new DataView(array.buffer);
...
export function SerializeApp(value) {
  if (value) {
    return SerializeBytes(value);
  } else {
    return new ArrayBuffer(0);
...
export class Participant {
  ...
  validate(compatible = false) {
    ...
    new SEC1EncodedPubKey(this.view.buffer.slice(offsets[3], offsets[4]), { validate: false }).validate();
  }
...
export function SerializeParticipant(value) {
  const buffers = [];
  buffers.push(SerializeByte32(value.payment_script_hash));
  buffers.push(SerializeUint64(value.payment_min_capacity));
  buffers.push(SerializeByte32(value.unlock_script_hash));
  buffers.push(SerializeSEC1EncodedPubKey(value.pub_key));
  return serializeTable(buffers);
}

export class ChannelParameters {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
  ...
  validate(compatible = false) {
    const offsets = verifyAndExtractOffsets(this.view, 0, true);
    new Participant(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
    new Participant(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
    new Byte32(this.view.buffer.slice(offsets[2], offsets[3]), { validate: false }).validate();
    new Uint64(this.view.buffer.slice(offsets[3], offsets[4]), { validate: false }).validate();
    new App(this.view.buffer.slice(offsets[4], offsets[5]), { validate: false }).validate();
    new Bool(this.view.buffer.slice(offsets[5], offsets[6]), { validate: false }).validate();
    new Bool(this.view.buffer.slice(offsets[6], offsets[7]), { validate: false }).validate();
  }
  ...
  getPartyA() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.getUint32(start + 4, true);
    return new Participant(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
  ...
  getApp() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.getUint32(start + 4, true);
    return new App(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
...
export function SerializeChannelParameters(value) {
  const buffers = [];
  buffers.push(SerializeParticipant(value.party_a));
  buffers.push(SerializeParticipant(value.party_b));
  buffers.push(SerializeByte32(value.nonce));
  buffers.push(SerializeUint64(value.challenge_duration));
  buffers.push(SerializeApp(value.app));
  buffers.push(SerializeBool(value.is_ledger_channel));
  buffers.push(SerializeBool(value.is_virtual_channel));
  return serializeTable(buffers);
}
...
export class ChannelConstants {
  ...
  validate(compatible = false) {
    const offsets = verifyAndExtractOffsets(this.view, 0, true);
    new ChannelParameters(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
    new Byte32(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
    if (offsets[3] - offsets[2] !== 1) {
      throw new Error(`Invalid offset for pfls_hash_type: ${offsets[2]} - ${offsets[3]}`)
    }
    new Uint64(this.view.buffer.slice(offsets[3], offsets[4]), { validate: false }).validate();
    new Byte32(this.view.buffer.slice(offsets[4], offsets[5]), { validate: false }).validate();
    if (offsets[6] - offsets[5] !== 1) {
      throw new Error(`Invalid offset for pcls_hash_type: ${offsets[5]} - ${offsets[6]}`)
    }
    new ChannelToken(this.view.buffer.slice(offsets[6], offsets[7]), { validate: false }).validate();
  }
  ...
  getParams() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.getUint32(start + 4, true);
    return new ChannelParameters(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
  ...
  getThreadToken() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.byteLength;
    return new ChannelToken(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
}

export function SerializeChannelConstants(value) {
  const buffers = [];
  buffers.push(SerializeChannelParameters(value.params));
  buffers.push(SerializeByte32(value.pfls_code_hash));
  const pflsHashTypeView = new DataView(new ArrayBuffer(1));
  pflsHashTypeView.setUint8(0, value.pfls_hash_type);
  buffers.push(pflsHashTypeView.buffer)
  buffers.push(SerializeUint64(value.pfls_min_capacity));
  buffers.push(SerializeByte32(value.pcls_code_hash));
  const pclsHashTypeView = new DataView(new ArrayBuffer(1));
  pclsHashTypeView.setUint8(0, value.pcls_hash_type);
  buffers.push(pclsHashTypeView.buffer)
  buffers.push(SerializeChannelToken(value.thread_token));
  return serializeTable(buffers);
}
...
export function SerializeFund(value) {
  const buffer = assertArrayBuffer(value);
  assertDataLength(buffer.byteLength, 1);
  return buffer;
}
...
export function SerializeAbort(value) {
  const buffer = assertArrayBuffer(value);
  assertDataLength(buffer.byteLength, 1);
  return buffer;
}
...
export function SerializeDispute(value) {
  const buffers = [];
  buffers.push(SerializeBytes(value.sig_a));
  buffers.push(SerializeBytes(value.sig_b));
  return serializeTable(buffers);
}
...
export class Close {
  ...
  validate(compatible = false) {
    const offsets = verifyAndExtractOffsets(this.view, 0, true);
    new ChannelState(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
    new Bytes(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
    new Bytes(this.view.buffer.slice(offsets[2], offsets[3]), { validate: false }).validate();
  }
  ...
  getState() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.getUint32(start + 4, true);
    return new ChannelState(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
...
export function SerializeClose(value) {
  const buffers = [];
  buffers.push(SerializeChannelState(value.state));
  buffers.push(SerializeBytes(value.sig_a));
  buffers.push(SerializeBytes(value.sig_b));
  return serializeTable(buffers);
}
...
export function SerializeForceClose(value) {
  const buffer = assertArrayBuffer(value);
  assertDataLength(buffer.byteLength, 1);
  return buffer;
}

export class ChannelWitness {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
  ...
  validate(compatible = false) {
    ...
    switch (t) {
    case 0:
      new Fund(this.view.buffer.slice(4), { validate: false }).validate();
      break;
    case 1:
      new Abort(this.view.buffer.slice(4), { validate: false }).validate();
      break;
    case 2:
      new Dispute(this.view.buffer.slice(4), { validate: false }).validate();
      break;
    case 3:
      new Close(this.view.buffer.slice(4), { validate: false }).validate();
      break;
    case 4:
      new ForceClose(this.view.buffer.slice(4), { validate: false }).validate();
      break;
    default:
      throw new Error(`Invalid type: ${t}`);
    }
  ...
  value() {
    ...
    switch (t) {
    case 0:
      return new Fund(this.view.buffer.slice(4), { validate: false });
    case 1:
      return new Abort(this.view.buffer.slice(4), { validate: false });
    case 2:
      return new Dispute(this.view.buffer.slice(4), { validate: false });
    case 3:
      return new Close(this.view.buffer.slice(4), { validate: false });
    case 4:
      return new ForceClose(this.view.buffer.slice(4), { validate: false });
    default:
      throw new Error(`Invalid type: ${t}`);
    }
  }
...
export function SerializeChannelWitness(value) {
  switch (value.type) {
  case "Fund":
    {
      const itemBuffer = SerializeFund(value.value);
      const array = new Uint8Array(4 + itemBuffer.byteLength);
      const view = new DataView(array.buffer);
      view.setUint32(0, 0, true);
      array.set(new Uint8Array(itemBuffer), 4);
  ...
  case "Abort":
    {
      const itemBuffer = SerializeAbort(value.value);
      const array = new Uint8Array(4 + itemBuffer.byteLength);
      const view = new DataView(array.buffer);
      view.setUint32(0, 1, true);
      array.set(new Uint8Array(itemBuffer), 4);
  ...
  case "Dispute":
    {
      const itemBuffer = SerializeDispute(value.value);
      const array = new Uint8Array(4 + itemBuffer.byteLength);
      const view = new DataView(array.buffer);
  ...
  case "Close":
    {
      const itemBuffer = SerializeClose(value.value);
      const array = new Uint8Array(4 + itemBuffer.byteLength);
      const view = new DataView(array.buffer);
  ...
  case "ForceClose":
    {
      const itemBuffer = SerializeForceClose(value.value);
      const array = new Uint8Array(4 + itemBuffer.byteLength);
      const view = new DataView(array.buffer);
...
export class ChannelState {
  ...
  validate(compatible = false) {
    const offsets = verifyAndExtractOffsets(this.view, 0, true);
    new Byte32(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
    new Balances(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
    new Uint64(this.view.buffer.slice(offsets[2], offsets[3]), { validate: false }).validate();
    new Bool(this.view.buffer.slice(offsets[3], offsets[4]), { validate: false }).validate();
  }
  ...
  getBalances() {
    ...
    const offset = this.view.getUint32(start, true);
    const offset_end = this.view.getUint32(start + 4, true);
    return new Balances(this.view.buffer.slice(offset, offset_end), { validate: false });
  }
...
export function SerializeChannelState(value) {
  const buffers = [];
  buffers.push(SerializeByte32(value.channel_id));
  buffers.push(SerializeBalances(value.balances));
  buffers.push(SerializeUint64(value.version));
  buffers.push(SerializeBool(value.is_final));
  return serializeTable(buffers);
}
...
export class ChannelStatus {
  ...
  validate(compatible = false) {
    const offsets = verifyAndExtractOffsets(this.view, 0, true);
    new ChannelState(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
    new Bool(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
...
export function SerializeChannelStatus(value) {
  const buffers = [];
  buffers.push(SerializeChannelState(value.state));
  buffers.push(SerializeBool(value.funded));
  buffers.push(SerializeBool(value.disputed));
  return serializeTable(buffers);
}
...
export class ChannelToken {
  ...
  validate(compatible = false) {
    assertDataLength(this.view.byteLength, ChannelToken.size());
    this.getOutPoint().validate(compatible);
  }
  static size() {
    return 0 + OutPoint.size();
  }
}

export function SerializeChannelToken(value) {
  const array = new Uint8Array(0 + OutPoint.size());
  const view = new DataView(array.buffer);
  array.set(new Uint8Array(SerializeOutPoint(value.out_point)), 0);
  return array.buffer;
}
